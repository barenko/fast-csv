...
extended(ParserStream).extend({
                ...
                self.__transform = function (data, cb) {
                    ...
                    if (data.length > headersLength) {
                        if (discardUnmappedColumns) {
                            ...
                            self.emit("parser-error", new Error("Unexpected Error: column header mismatch expected: " + headersLength + " columns got: " + data.length));
                            return orig(null, cb);
                        }
    ...
    validate: function (cb) {
        if (!extended.isFunction(cb)) {
            this.emit("parser-error", new TypeError("fast-csv.Parser#validate requires a function"));
        }
        if (cb.length === 2) {
            this.__validate = cb;
        } else {
    ...
    transform: function (cb) {
        if (!extended.isFunction(cb)) {
            this.emit("parser-error", new TypeError("fast-csv.Parser#transform requires a function"));
        }
        if (cb.length === 2) {
            this.__transform = cb;
        } else {
